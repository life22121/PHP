                                                ...
                                                <span class="float-right" style="position: relative;top: 7px;">
                                                    @if($wallet->balance > 0)
                                                        @if ($wallet->currency->type != 'fiat')
                                                            <span class="text-success">{{ $wallet->currency->symbol }}{{$wallet->balance }}</span>
                                                        @else
                                                            <span class="text-success">{{ $wallet->currency->symbol }}{{formatNumber($wallet->balance) }}</span>
                                                        @endif
                                                    @elseif($wallet->balance == 0)
                                                        @if ($wallet->currency->type != 'fiat')
                                                            <span>{{ $wallet->balance }}</span>
                                                        @else
                                                            <span>{{ $wallet->currency->symbol }}{{ formatNumber($wallet->balance) }}</span>
                                                        @endif
                                                    @elseif($wallet->balance < 0)
                                                        @if ($wallet->currency->type != 'fiat')
                                                            <span class="text-danger">{{ $wallet->balance }}</span>
                                                        @else
                                                            <span class="text-danger">{{ $wallet->currency->symbol }}{{formatNumber($wallet->balance) }}</span>
                                                        @endif
                                                    @endif
                                                </span>
                                            </div>
